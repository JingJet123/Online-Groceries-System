...
public class AddStaff extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        try {
            ...
            Staff stf = new Staff(username, password, stfName, contact, email, salary, role, buffer.toByteArray());

            List<Staff> stfList = (List<Staff>) getServletContext().getAttribute("stfList");
            stfList.add(stf);
            getServletContext().setAttribute("stfList", stfList);
            //Add to database
            StaffService stfService = new StaffService(em);
            utx.begin();
            boolean success = stfService.addStaff(stf);
            utx.commit();
            ...
            if (success) {
                ...
                session.setAttribute("alertMsg", stf.getUsername() + " Successfully Registered.");
